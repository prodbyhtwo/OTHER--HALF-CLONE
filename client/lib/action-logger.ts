...
class ActionLogger {
  ...
  /**
   * Log dead button detection
   */
  logDeadButton(elementPath: string, outerHTML: string, reason: string): void {
    // Only log dead buttons in development to reduce noise
    ...
    this.log(
      "ui.control_without_handler",
      "warn",
      `Dead button detected: ${elementPath}`,
      {
        element_path: elementPath,
        outer_html: outerHTML.substring(0, 100), // Reduce to 100 chars
        reason,
      },
    );
  }
  ...
  private async flush(): Promise<void> {
    ...
    try {
      ...
      this.buffer.unshift(
        ...logsToSend.slice(0, this.maxBufferSize - this.buffer.length),
      );
...
export function useActionLogger() {
  return {
    ...
    logDeadButton: actionLogger.logDeadButton.bind(actionLogger),
