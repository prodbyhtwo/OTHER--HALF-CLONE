...
const Header = ({ userName }: { userName: string }) => {
  ...
  return (
    <div className="flex items-center justify-between px-6 py-3 bg-white">
      ...
      <h1 className="text-2xl font-bold text-black">Report {userName}</h1>
...
const RadioOption = ({
  ...
  >
    <div className="relative">
      <div className="w-6 h-6 border-2 border-[#9610FF] rounded-full flex items-center justify-center">
        {checked && <div className="w-3.5 h-3.5 bg-[#9610FF] rounded-full" />}
      </div>
    </div>
    <span className="text-xl text-black font-medium">{label}</span>
  </button>
);
...
export default function ReportUser() {
  ...
  const navigate = useNavigate();
  const userName = "Monica"; // This could come from props or URL params
