...
const relationshipGoals: RelationshipGoal[] = [
  {
    id: "dating",
    title: "Dating 👩‍❤️‍👨",
    description:
...
export default function OnboardingGoals() {
  const [selectedGoal, setSelectedGoal] = useState("dating");
  const navigate = useNavigate();

  const handleContinue = () => {
    navigate("/onboarding/distance");
  };
    ...
    <div className="min-h-screen bg-white flex flex-col max-w-[430px] mx-auto">
      ...
      <div className="flex items-center justify-between px-6 py-3 h-18">
        ...
        <div className="flex-1 mx-4 h-3 bg-gray-200 rounded-full">
          <div className="w-1/2 h-full bg-[#9610FF] rounded-full"></div>
        </div>
      ...
      <div className="flex-1 px-6 py-3 overflow-y-auto">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500 text-lg leading-[29px] tracking-wide">
            Choose the type of relationship you're seeking on Datify. Love,
            friendship, or something in between—it's your choice.
          </p>
        </div>
            ...
            <button
              ...
              className={`w-full p-3 rounded-xl text-left transition-all border-2 ${
                selectedGoal === goal.id
                  ? "border-[#9610FF] bg-white"
                  : "border-gray-200 bg-white hover:border-gray-300"
              }`}
            >
              <div className="mb-2">
                <h3 className="text-gray-900 text-xl font-bold leading-8">
                  {goal.title}
                </h3>
              </div>
              <p className="text-gray-500 text-base leading-6 tracking-wide">
                {goal.description}
              </p>
            </button>
          ))}
      ...
      <div className="p-6 pt-6 pb-9 border-t border-gray-100 bg-white">
        <button
          ...
          className="w-full py-4 rounded-full bg-[#9610FF] text-white text-base font-bold hover:bg-[#8510E6] transition-colors"
        >
          Continue
        </button>
      </div>
