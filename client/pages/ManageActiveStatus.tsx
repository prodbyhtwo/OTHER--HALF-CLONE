...
const ManageActiveStatus: React.FC = () => {
  const navigate = useNavigate();
  const [showActiveStatus, setShowActiveStatus] = useState(true);
  const [showRecentlyActive, setShowRecentlyActive] = useState(false);
        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between w-full">
            <div className="flex-1">
              ...
              <p className="text-base text-gray-600">
                If you are active on Datify, your status will be displayed to
                other people.
              </p>
            </div>
            <div className="pt-1">
              <button
                ...
                className={`w-11 h-6 rounded-full transition-colors ${
                  showActiveStatus ? "bg-purple-600" : "bg-gray-300"
                }`}
              >
                <div
                  className={`w-5 h-5 bg-white rounded-full transition-transform ${
                    showActiveStatus ? "translate-x-5" : "translate-x-0.5"
                  }`}
                />
              </button>
            </div>
          ...
          <div className="flex items-start justify-between w-full">
            <div className="flex-1">
              ...
              <p className="text-base text-gray-600">
                Recently active status will be displayed if you have been active
                on Datify in the last 24 hours.
              </p>
            </div>
            <div className="pt-1">
              <button
                onClick={() => setShowRecentlyActive(!showRecentlyActive)}
                className={`w-11 h-6 rounded-full transition-colors ${
                  showRecentlyActive ? "bg-purple-600" : "bg-gray-300"
                }`}
              >
                <div
                  className={`w-5 h-5 bg-white rounded-full transition-transform ${
                    showRecentlyActive ? "translate-x-5" : "translate-x-0.5"
                  }`}
                />
              </button>
            </div>
...
export default ManageActiveStatus;
