  ...
  send(data: string): void {
    try {
      ...
      if (message.type === 'subscribe') {
        ...
        console.log(`🔔 [MOCK] Subscribed to updates for user ${message.userId}`);
        ...
        console.log(`🔕 [MOCK] Unsubscribed from updates for user ${message.userId}`);
      }
    } catch (error) {
  ...
  private startMockUpdates(): void {
    ...
    this.updateInterval = setInterval(() => {
      this.subscriptions.forEach(userId => {
        if (this.onmessage && Math.random() > 0.8) { // 20% chance of update
          const updateTypes = ['profile_update', 'settings_update', 'location_update'];
...
export function useRealtime(
  ...
  const connect = useCallback(() => {
    ...
    try {
      ...
      const isDevelopment = import.meta.env.DEV || import.meta.env.VITE_SAFE_MODE === 'true';
      
      if (isDevelopment) {
        wsRef.current = new MockWebSocket('ws://localhost:mock/realtime');
      } else {
        const wsUrl = import.meta.env.VITE_REALTIME_WS_URL || 'wss://api.yourapp.com/realtime';
...
export function useRealtimeUpdates(
  ...
  const [updatesByType, setUpdatesByType] = useState<Record<string, RealtimeUpdate[]>>({});

  useEffect(() => {
    if (lastUpdate && updateTypes.includes(lastUpdate.type)) {
      setUpdatesByType(prev => ({
        ...prev,
        [lastUpdate.type]: [lastUpdate, ...(prev[lastUpdate.type] || []).slice(0, 9)]
      }));
    }
