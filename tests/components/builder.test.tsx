...
describe('Builder Components', () => {
  ...
  describe('BlockUserButton', () => {
    ...
    it('shows confirmation dialog when confirmation is required', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByText('Are you sure you want to block this user?')).toBeInTheDocument();
      });
    });
  ...
  describe('LocationShare', () => {
    ...
    it('requests location when button is clicked', async () => {
      const getCurrentPositionSpy = vi.spyOn(global.navigator.geolocation, 'getCurrentPosition');
    ...
    it('allows manual address entry', async () => {
      ...
      await waitFor(() => {
        const addressInput = screen.getByPlaceholderText(/enter your city or address/i);
