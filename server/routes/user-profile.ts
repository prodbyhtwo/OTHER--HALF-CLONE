...
function sanitizeUserForPublic(user: User): Partial<User> {
  const { 
    id, full_name, age, bio, denomination, church_attendance, 
...
router.put("/me/location", authenticateToken, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
...
router.post("/users/:id/block", authenticateToken, async (req: Request, res: Response) => {
  try {
...
router.delete("/users/:id/block", authenticateToken, async (req: Request, res: Response) => {
  try {
    const currentUser = (req as any).user;
    const { id: targetUserId } = req.params;
...
router.get("/me/blocks", authenticateToken, async (req: Request, res: Response) => {
  try {
    const currentUser = (req as any).user;
    const user = findUserById(currentUser.id);
