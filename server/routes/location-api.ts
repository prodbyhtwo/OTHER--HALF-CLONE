...
const locationPermissionSchema = z
  .object({
    granted: z.boolean(),
...
async function geocodeAddress(address: string): Promise<{
  ...
  try {
    ...
    if (lowerAddress.includes("new york")) {
      return { lat: 40.7128, lng: -74.006, accuracy: 100 };
...
function updateLocationData(
  userId: string,
  data: Partial<(typeof mockLocationData)[0]>,
) {
