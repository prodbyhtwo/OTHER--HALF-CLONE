...
const discoveryPreferencesSchema = z
  .object({
    ...
    preferred_church_attendance: z
      .array(
        z.enum(["weekly", "monthly", "occasionally", "holidays_only", "never"]),
      )
      .optional(),
  })
  .strict();
...
function updateUserSettings(
  ...
  const settingsIndex = mockUserSettings.findIndex(
...
function validateSettingsConsistency(
  req: Request,
  res: Response,
  next: Function,
) {
  ...
  if (updates.email_preferences?.security === false) {
    return res.status(400).json({
      success: false,
      error:
        "Security email notifications are strongly recommended and cannot be disabled",
    });
  }
...
router.put(
  ...
  async (req: Request, res: Response) => {
    try {
      ...
      if (validatedUpdates.push_preferences)
        changedFields.push("push_preferences");
      if (validatedUpdates.email_preferences)
        changedFields.push("email_preferences");
      if (validatedUpdates.privacy_preferences)
        changedFields.push("privacy_preferences");
      if (validatedUpdates.discovery_preferences)
        changedFields.push("discovery_preferences");
