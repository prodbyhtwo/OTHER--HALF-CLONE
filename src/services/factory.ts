...
export function createMailer(): Mailer {
  ...
  if (env.SAFE_MODE || !env.SENDGRID_API_KEY || !env.FROM_EMAIL) {
    console.log(
      "📧 Using MockMailer (SAFE_MODE or missing SENDGRID_API_KEY/FROM_EMAIL)",
    );
...
export function createPayments(): Payments {
  ...
  if (env.SAFE_MODE || !env.STRIPE_SECRET_KEY) {
    console.log(
      "💳 Using MockPayments (SAFE_MODE or missing STRIPE_SECRET_KEY)",
    );
...
export function createStorage(): Storage {
  ...
  if (env.SAFE_MODE || !env.STORAGE_BUCKET_NAME || !env.STORAGE_ACCESS_KEY) {
    console.log(
      "🗄️  Using MockStorage (SAFE_MODE or missing storage credentials)",
    );
...
export function createAnalytics(): Analytics {
  ...
  if (env.SAFE_MODE || !env.ANALYTICS_API_KEY) {
    console.log(
      "📊 Using MockAnalytics (SAFE_MODE or missing ANALYTICS_API_KEY)",
    );
...
export function createPushNotifications(): PushNotifications {
  ...
  if (env.SAFE_MODE || !env.PUSH_NOTIFICATION_SERVER_KEY) {
    console.log(
      "🔔 Using MockPushNotifications (SAFE_MODE or missing PUSH_NOTIFICATION_SERVER_KEY)",
    );
...
export function getServiceStatus() {
  return {
    ...
    pushNotifications:
      env.SAFE_MODE || !env.PUSH_NOTIFICATION_SERVER_KEY ? "mock" : "real",
