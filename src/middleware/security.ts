...
const createSlowDown = (windowMs: number, delayAfter: number, maxDelayMs: number = 30000) => {
  return slowDown({
    windowMs,
    delayAfter,
    maxDelayMs,
...
export function configureSecurityMiddleware(app: Application): void {
  ...
  app.use("/api/invites", createRateLimiter(
    60 * 60 * 1000, // 1 hour
  ...
  app.use("/api/stripe/webhook", createRateLimiter(
    60 * 1000, // 1 minute
...
export function logPaymentEvent(event: string, metadata?: Record<string, any>) {
  return (req: Request, res: Response, next: NextFunction) => {
    logSecurityEvent(`payment_${event}`, {
      ...metadata,
      success: res.statusCode < 400,
    }, req);
    next();
